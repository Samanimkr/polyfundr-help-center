...
<Note>
  **You can only open between 10¢ and 90¢.** An entry outside that band is
  refused when you place it, and nothing happens to your account. Closing is not
  restricted: a position that runs to 97¢ can still be closed at 97¢. See
  [Challenge rules](/challenges/rules).
</Note>
