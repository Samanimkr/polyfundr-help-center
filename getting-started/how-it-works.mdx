...
description: "A step-by-step breakdown of the PolyFundr flow: buy a challenge, pass it, get a funded account, and withdraw your profits in USDC."
---
...
```mermaid
flowchart TD
    A["Buy challenge"] --> B["Pass challenge<br/>+30% equity, profit spread across days"]
    B --> C["Funded account opens<br/>same account size"]
    C --> D["Reach payout threshold<br/>2% realized eligible profit"]
...
| Consistency          | No single trading day over your account's cap (30%, or 50% upgraded) of the winning days' combined profit |
...
- Payouts use an 80% split, or 90% if you bought the [upgrade](/challenges/upgrades).
- Payouts are sent in USDC on Polygon.
- You must enter the destination wallet address every time.
- Payout requests use progressive caps: 5% of your account on the 1st request,
  then 10%, up to \$5,000 per payout.
